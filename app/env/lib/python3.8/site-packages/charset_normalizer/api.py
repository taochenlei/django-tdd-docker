...
from charset_normalizer.constant import (IANA_SUPPORTED, TOO_BIG_SEQUENCE,
                                         TOO_SMALL_SEQUENCE)
from charset_normalizer.md import mess_ratio
from charset_normalizer.models import CharsetMatch, CharsetMatches
from charset_normalizer.utils import (any_specified_encoding, iana_name,
                                      identify_sig_or_bom, is_cp_similar,
...
def from_bytes(
    ...
    if len(results) == 0:
        ...
        elif (fallback_u8 and fallback_ascii is None) or (
            fallback_u8 and fallback_u8.fingerprint != fallback_ascii.fingerprint
        ):
...
def normalize(
    ...
    with open(
        "{}".format(path.replace(filename, "".join(target_extensions))), "wb"
    ) as fp:
