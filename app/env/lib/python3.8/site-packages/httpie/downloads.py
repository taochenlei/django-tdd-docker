...
CLEAR_LINE = "\r\033[K"
PROGRESS = (
    "{percentage: 6.2f} %" " {downloaded: >10}" " {speed: >10}/s" " {eta: >8} ETA"
)
...
def parse_content_range(content_range: str, resumed_from: int) -> int:
    ...
    if first_byte_pos >= last_byte_pos or (
        ...
        raise ContentRangeError("Invalid Content-Range returned: %r" % content_range)
    ...
    if first_byte_pos != resumed_from or (
        ...
        raise ContentRangeError(
            ...
            ' for the requested Range ("bytes=%d-")' % (content_range, resumed_from)
        )
...
class Downloader:
    def __init__(
        ...
        self._progress_reporter = ProgressReporterThread(
            status=self.status, output=progress_file
        )
    ...
    def start(
        ...
        self.status.started(resumed_from=self._resumed_from, total_size=total_size)
        ...
        self._progress_reporter.output.write(
            'Downloading %sto "%s"\n'
            % (
                (humanize_bytes(total_size) + " " if total_size is not None else ""),
...
class ProgressReporterThread(threading.Thread):
    ...
    def __init__(self, status: DownloadStatus, output: IO, tick=0.1, update_interval=1):
        super().__init__()
        self.status = status
        self.output = output
        self._tick = tick
    ...
    def report_speed(self):
        ...
        self.output.write(
            CLEAR_LINE + " " + SPINNER[self._spinner_pos] + " " + self._status_line
        )
        self.output.flush()

        self._spinner_pos = (
            self._spinner_pos + 1 if self._spinner_pos + 1 != len(SPINNER) else 0
        )
    ...
    def sum_up(self):
        ...
        self.output.write(
            SUMMARY.format(
                ...
                total=(
                    self.status.total_size and humanize_bytes(self.status.total_size)
                ),
                speed=humanize_bytes(speed),
                time=time_taken,
            )
