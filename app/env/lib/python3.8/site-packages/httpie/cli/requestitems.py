...
from httpie.cli.constants import (SEPARATOR_DATA_EMBED_FILE_CONTENTS,
                                  ...
                                  SEPARATOR_DATA_STRING, SEPARATOR_FILE_UPLOAD,
...
from httpie.cli.dicts import (MultipartRequestDataDict, RequestDataDict,
                              RequestFilesDict, RequestHeadersDict,
                              RequestJSONDataDict, RequestQueryParamsDict)
...
def process_empty_header_arg(arg: KeyValueArg) -> str:
    if arg.value:
        raise ParseError(
            'Invalid item "%s" ' "(to specify an empty header use `Header;`)" % arg.orig
        )
    return arg.value
...
def load_text_file(item: KeyValueArg) -> str:
    ...
    try:
        ...
        raise ParseError(
            ...
            " not a UTF8 or ASCII-encoded text file" % (item.orig, item.value)
        )
