...
"""
from argparse import OPTIONAL, SUPPRESS, ZERO_OR_MORE, FileType
...
from httpie.cli.argtypes import (KeyValueArgType, SessionNameValidator,
                                 readable_file_arg)
from httpie.cli.constants import (DEFAULT_FORMAT_OPTIONS, OUT_REQ_BODY,
...
from httpie.output.formatters.colors import (AUTO_STYLE, AVAILABLE_STYLES,
                                             DEFAULT_STYLE)
...
content_type = parser.add_argument_group(
    title="Predefined Content Types", description=None
)
...
content_processing = parser.add_argument_group(
    title="Content Processing Options", description=None
)
...
output_processing.add_argument(
    ...
    """.format(
        option_list="\n".join(
            (8 * " ") + option for option in DEFAULT_FORMAT_OPTIONS
        ).strip()
    ),
)
...
sessions = parser.add_argument_group(title="Sessions").add_mutually_exclusive_group(
    required=False
)
